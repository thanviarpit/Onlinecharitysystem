...
export class DonorControlComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.ngoSvc.getAllNgos().subscribe(
      ...
      // this.donationsub=this.donationSvc.getDonationUpdatedListener().
      // subscribe((newdonations : Donation[])=>this.donas=newdonations);

      this.donationsub=this.donationSvc.getDonationUpdatedListener().
      subscribe((newdonations : Donation[])=>console.log(newdonations));

      //console.log(this.donas);
  }
